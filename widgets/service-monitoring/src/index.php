...
while ($row = $res->fetchRow()) {
    foreach ($row as $key => $value) {
        if ($key == "last_check") {
            ...
            $replacenya   = array("", " "," "," "," "," "," ","","","","","","","","");
            $value = str_replace($findnya, $replacenya, substr($value, 0, $outputLength));//str_replace("","",substr($value, 0, $outputLength));
...
//edited
    /*
if (strpos($row['output'], 'alert') !== false){
    $findnya = array("/opt/mnp/flags/tftFlag/", "\r\n","\r","\n","\\r","\\n","\\r\\n","/opt/mnp/flags/ibisFlag/","/opt/mnp/flags/miscFlag/","/opt/mnp/flags/ntpFlag/","alert_","alert-","/opt/mnp/flags/ledFlag/"," geen alarm","geen alarm");
    $replacenya   = array("", " "," "," "," "," "," ","","","",",",",","","","");
    $output=str_replace($findnya, $replacenya,$row['output']);
    $outputexp=array_unique(explode(",", $output));
    $output=implode(",", $outputexp);
    $busloc=explode("-", $row["description"]);
    ...
    $lastchk = date("Y-m-d H:i:s",$row["last_check"]);
   // $sel=$MNPrack->query("SELECT * FROM daily_alert WHERE bus_id='".$row['hostname']."' AND alert='".trim(substr($output,1))."' AND date LIKE '%".date("Y-m-d")."%';");
    if($sel->numRows()==0){
         //$MNPrack->query("INSERT INTO `mnp_rack`.`daily_alert` (`bus_id`,`bus_locate`, `alert`,`is_recording`,`is_nvs_offline`,`is_ssd`,`is_pftlux`,`is_pfttemp`,`is_chkfilesize`, `date`,`lastcheck`) VALUES ('".$row['hostname']."','".$busloc[1]."', '".trim(substr($output,1))."', '".$record."', '".$nvs."', '".$ssd."', '".$pftlux."', '".$pfttemp."', '".$chkfilesize."','".$lastchk."','".$lastchk."'); ");
    }else{
       // $MNPrack->query("UPDATE `mnp_rack`.`daily_alert` SET `lastcheck` = '".$lastchk."' WHERE bus_id='".$row['hostname']."' AND alert='".trim(substr($output,1))."' AND date LIKE '%".date("Y-m-d")."%';");
    }
}
//end edited
*/
