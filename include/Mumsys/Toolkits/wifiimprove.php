...
<script type="text/javascript" src="include/common/datatable.js"></script>
		<script type="text/javascript">
			jQuery(document).ready(function($) {				
				...
				if($("#selbusid option:selected").text()=="-Select Concession-" || $("#selbusid option:selected").val()=="-Select BUS ID-"){
					alert("Please Select Bus Data First!");
					return false;
				}
				...
				$("#btngo").attr('disabled','disabled');
					 $.post("include/Mumsys/Toolkits/act_toolkits.php",
			          ...
			          function(data, status){
			          	...
			          	$("#btngo").attr('value', 'Wi-Fi Change!');
			          });
				});

				$(document).on("click","#btnmodemx",function(){					
				if($("#selbusid option:selected").text()=="-Select Concession-" || $("#selbusid option:selected").val()=="-Select BUS ID-"){
					alert("Please Select Bus Data First!");
					return false;
				}
				$("#btnmodemx").attr('value', 'Please Wait...');
				$("#btnmodemx").attr('disabled','disabled');
					 $.post("include/Mumsys/Toolkits/act_toolkits.php",
			          {
			          	method:"execwifienmodemx",
			          	busid:$("#selbusid option:selected").text(),
			          	busip:$("#selbusid option:selected").val(),
			          	hgroup:$("#selconces option:selected").val(),
			          },
			          function(data, status){
			          	$('#example').DataTable().ajax.reload();
			          	alert(data);
			          	$("#btnmodemx").removeAttr('disabled');
			          	$("#btnmodemx").attr('value', 'Modemx Enable!');
			          });
				});
				 ...
				 function loadtable(ajaxurl){
			        ...
			        "ajax": ajaxurl+"?loadmeth=wifiimproveload",
			        "columns": [
			            {
			                "orderable":      false,
			                "data":           null,
...
<table id='tbwifiimprove'>
	<tr>
		...
		<td><input type="button" name="btngo" id="btngo" value="Wi-Fi Change!"></td>
		<td><input type="button" name="btnmodemx" id="btnmodemx" value="Modemx Enable!"></td>
	</tr>
</table>
<br><br>
