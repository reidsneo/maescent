 ...
 */
	...
	if ($rows != 0) {
		...
		for ($i = $istart; $i <= $iend; $i++)
			$pageArr[$i] = array("url_page"=>"./main.php?p=".$p.$mini."&num=$i&limit=".$limit."&poller=".$poller."&template=$template&search=".$search."&type=".$type."&o=" . $o . $url_var, "label_page"=>"<b>".($i +1)."</b>","num"=> $i);
		...
		if (($prev = $num - 1) >= 0)
			$tpl->assign('pagePrev', ("./main.php?p=".$p.$mini."&num=$prev&limit=".$limit."&poller=".$poller."&template=$template&search=".$search."&type=".$type."&o=" . $o .$url_var));

		if (($next = $num + 1) < ($rows/$limit))
			$tpl->assign('pageNext', ("./main.php?p=".$p.$mini."&num=$next&limit=".$limit."&poller=".$poller."&template=$template&search=".$search."&type=".$type."&o=" . $o .$url_var));
		...
		if ($page_max > 5 && $num != 0)
			$tpl->assign('firstPage', ("./main.php?p=".$p.$mini."&num=0&limit=".$limit."&poller=".$poller."&template=$template&search=".$search."&type=".$type."&o=" . $o .$url_var));
		if ($page_max > 5 && $num != ($pageNumber-1))
			$tpl->assign('lastPage', ("./main.php?p=".$p.$mini."&num=".($pageNumber-1)."&limit=".$limit."&template=$template&poller=".$poller."&search=".$search."&type=".$type."&o=" . $o .$url_var));
	...
	<?php
	$form = new HTML_QuickForm('select_form', 'GET', "?p=".$p.$mini."&search_type_service=" . $search_type_service."&search_type_host=" . $search_type_host);
