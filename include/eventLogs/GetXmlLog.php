...
$req = "SELECT SQL_CALC_FOUND_ROWS *,(SELECT services.output FROM services WHERE service_id=logs.service_id AND services.`host_id`=logs.`host_id` AND services.`description`=logs.`service_description`) AS outputfull FROM logs ".$innerJoinEngineLog." WHERE ctime > '$start' AND ctime <= '$end' $whereOutput $msg_req";
/*
 * Add Host
 */
$str_unitH = "";
...
if (isset($req) && $req) {
    ...
    while ($log = $DBRESULT->fetchRow()) {
        ...
        $buffer->writeElement("contact", $log["outputfull"], false); //edited contact
...
$buffer->writeElement("lo", _("ext Output"), 0); 
$buffer->writeElement("c", _("Extended Output"), 0);//edited contact
