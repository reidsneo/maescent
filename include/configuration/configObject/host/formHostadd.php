...
if ($valid) {
    ...
    echo 'Host has been changed, to take the effect please refresh configuration file by click button below<br>
    <input id="sbmtrefresh" type="button" value="Refresh!">
    <input id="sessid" type="hidden" value="'.session_id().'">
    <div id="msgresp" name="msgresp"></div>';
echo "<pre>";
//$seltpl=['tpSelect']['#index#']
$submited=$form->getSubmitValues();
$serverid=$submited['nagios_server_id'];
//echo var_dump($form);
echo "</pre>";
echo '<script type="text/javascript">
        $(document).on("click","#sbmtrefresh",function(){
            jQuery("#sbmtrefresh").val("Refreshing Poller, please wait...");
                jQuery.ajax({
                    type: "POST",
                    url: "include/configuration/configGenerate/xml/generateFiles.php",
                    data: {"sid":jQuery("#sessid").val(),"comment":false,"debug":true,"generate":true,"poller":'.$serverid.'},
                    success: function(xml){
                        jQuery.ajax({
                            type: "POST",
                            url: "include/configuration/configGenerate/xml/moveFiles.php",
                            data: {"sid":jQuery("#sessid").val(),"poller":'.$serverid.'},
                            success: function(move){
                                jQuery.ajax({
                                    type: "POST",
                                    url: "include/configuration/configGenerate/xml/restartPollers.php",
                                    data: {"sid":jQuery("#sessid").val(),"mode":1,"poller":'.$serverid.'},
                                    success: function(restart){
                                       jQuery("#msgresp").html("<font color=\"green\">Poller has been refreshed!</font>");
                                }
                            });
                        }
                    },"html");
