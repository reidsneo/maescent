...
for ($i = 0; $host = $DBRESULT->fetchRow(); $i++) {
	if (!isset($poller) || $poller == 0 || ($poller != 0 && $poller == $tab_relation_id[$host["host_id"]])) {
		...
		if ($host["host_activate"])
			$moptions = "<a href='main.php?p=".$p."&host_id=".$host['host_id']."&o=u&limit=".$limit."&num=".$num."&search=".$search.$mini."'><img src='img/icons/disabled.png' class='ico-14 margin_right' border='0' alt='"._("Disabled")."'></a>";
		else
			$moptions = "<a href='main.php?p=".$p."&host_id=".$host['host_id']."&o=s&limit=".$limit."&num=".$num."&search=".$search.$mini."'><img src='img/icons/enabled.png' class='ico-14 margin_right' border='0' alt='"._("Enabled")."'></a>";
		...
		if (count($tplArr)) {
			...
			foreach($tplArr as $key =>$value) {
				if ($firstTpl) {
					$tplStr .= "<a href='main.php?p=60103&o=c&host_id=".$key.$mini."'>".$value."</a>";
					$firstTpl = 0;
				} else
					$tplStr .= "&nbsp;|&nbsp;<a href='main.php?p=60103&o=c&host_id=".$key.$mini."'>".$value."</a>";
			}
